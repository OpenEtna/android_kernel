...
struct timer_list lg_enhanced_sliedon;
...
struct synaptics_ts_data {
	...
	uint32_t flags;
	int (*power) (int on);
	unsigned int leftkey_pressed;
	unsigned int rightkey_pressed;
			      ...
			      const struct i2c_device_id *id)
{
	...
	set_bit(KEY_HOME /*KEY_MENU */ , ts->input_dev->keybit);
	...
	is_menutimer_initialized = 1;
	is_slideon_timer_initialized = 1;
#ifdef CONFIG_HAS_EARLYSUSPEND
