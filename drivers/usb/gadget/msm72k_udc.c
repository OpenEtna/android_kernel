...
#include <mach/rpc_charger.h> //LnT fix
...
static void usb_chg_detect(struct work_struct *w)
{
	struct usb_info *ui = container_of(w, struct usb_info, chg_det.work);
	   struct msm_otg *otg = to_msm_otg(ui->xceiv); //LnT fix
	...
	hsusb_chg_connected(temp);
	    atomic_set(&otg->chg_type, temp); //LnT fix
	...
	for (;;) {
		...
		case USB_STATE_IDLE:
			if (flags & USB_FLAG_START) {
				...
				lge_battery_update_charger();/*LnT USB charging issue fix*/
			}
			break;
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				...
				atomic_set(&ui->running, 0);
				atomic_set(&ui->remote_wakeup, 0);
/* LGE_CHANGE_S, [kundan.thakur@lge.com], 28.10.2010*/
/* Fix for posting USB disconnect event to port-bridge/DunService when DUN is going on */
				atomic_set(&ui->configured, 0);
/* LGE_CHANGE_E, [kundan.thakur@lge.com], 28.10.2010*/
		...
		case USB_STATE_OFFLINE:
			...
			if ((flags & USB_FLAG_VBUS_ONLINE) && _vbus) {
				...
				schedule_delayed_work(
						&ui->chg_det,
						USB_CHG_DET_DELAY);
//LGE_CHANGE_S .. sinjo.mattappallil@lge.com 25/OCT/2010						
				/* ALESSI : LGE_CHANGE_S [euikyeom.kim@lge.com] 2010-08-09, Change USB state */
				...
				/* LGE_CHANGE_E [euikyeom.kim@lge.com] 2010-08-09 */
			}
...
//LGE_CHANGE_E .. sinjo.mattappallil@lge.com 25/OCT/2010			
			break;
		}
	}
}
