...
static int msm_otg_suspend(struct msm_otg *dev)
{
	...
	/*Work arounds in case of Integrated phy */
	if (!dev->pdata->config_gpio) {
		ulpi_read(dev, 0x14);/* clear PHY interrupt latch register */
		/* If there is no pmic notify support
	...
	/* In case of external phy,disable the GPIO the lines
	 * before going to LPM to avoid high power consumption
	 */
	if (dev->pdata->config_gpio) {
		if (dev->pdata->config_gpio(1)) {
			/* error fatal: It should never happen*/
			pr_err("%s: failed to disable gpio lines\n", __func__);
			goto out;
		}
		if (dev->gpio_vbus_irq)
			enable_irq(dev->gpio_vbus_irq);
	}
...
static int msm_otg_resume(struct msm_otg *dev)
{
	...
	/* In case of external phy, enable GPIO lines once USB is wakeup*/
	if (dev->pdata->config_gpio)
		dev->pdata->config_gpio(0);
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	if (dev->pdata->config_gpio) {
		...
		/* Request the gpio lines to use for USB in case of
		 * external phy
		 */
		if (dev->pdata->gpio_init)
			dev->pdata->gpio_init(1);

		/*Enable the GPIO lines which will use as ULPI signals
		 * in case of external phy
		 */
		dev->pdata->config_gpio(0);


		gpio_vbus_irq = platform_get_irq_byname(pdev, "vbus_interrupt");
		if (gpio_vbus_irq < 0) {
		...
		ret = request_irq(gpio_vbus_irq,
				&usb_lpm_gpio_isr,
				IRQF_TRIGGER_HIGH,
				"vbus_interrupt", NULL);
		if (ret) {
			pr_err("%s: failed to request vbus interrupt:(%d)\n",
					__func__, gpio_vbus_irq);
			goto free_regs;
		}
		dev->gpio_vbus_irq = gpio_vbus_irq;
		disable_irq(dev->gpio_vbus_irq);
...
free_vbus_irq:
	if (dev->gpio_vbus_irq)
		free_irq(dev->gpio_vbus_irq, dev);
free_regs:
	iounmap(dev->regs);
put_phy_clk:
...
static int __exit msm_otg_remove(struct platform_device *pdev)
{
	...
	if (dev->gpio_vbus_irq)
		free_irq(dev->gpio_vbus_irq, dev);
	/* Free the GPIO lines used by USB in case external phy*/
	if (dev->pdata->gpio_init)
		dev->pdata->gpio_init(0);
	/*Disable the GPIO lines in case of external phy */
	if (dev->pdata->config_gpio)
		dev->pdata->config_gpio(1);
	iounmap(dev->regs);
	if (dev->hs_cclk) {
		clk_disable(dev->hs_cclk);
		clk_put(dev->hs_cclk);
