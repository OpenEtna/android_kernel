...
#include <linux/proximity.h>//linking form compass to hall 
...
static int akm8973_config_gpio(int config)
{
	...
	if (config) {	// for wake state
		...
		//gpio_tlmm_config(GPIO_CFG(80, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
		gpio_tlmm_config(GPIO_CFG(80, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
...
static int akm8973_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	//disable_irq_nosync(this_client->irq); //L&T: Fix, deadlock in synchronize_irq
